...
#from sample_resource.models import SampleResource as SampleResourceModel, SampleResourceAggregate as SampleResourceAggregateModel
from vtplanner.controller.resource import VtPlanner as VtPlannerController
from vtplanner.forms.VtPlanner import VtPlanner as VtPlannerModelForm
from vtplanner.models import VtPlanner as VtPlannerModel
...
def create_resource(request, slice_id, agg_id):
    ...
    if request.method == "POST":
        ...
        if 'create_resource' in request.POST:
            return HttpResponseRedirect(reverse("vtplanner_resource_crud", args=[slice_id, agg_id]))
        else:
            return HttpResponseRedirect(reverse("slice_detail", args=[slice_id]))
...
def resource_crud(request, slice_id, agg_id, resource_id = None):
    ...
    def pre_save(instance, created):
        ...
        """
        instance = VtPlannerController.fill(instance, slice, agg_id, resource_id)

    try:
        return generic_crud(request, obj_id=resource_id, model=VtPlannerModel,
                 form_class=VtPlannerModelForm,
                 ...
                 ("%s SampleResource" % "Update" if resource_id else "Create", reverse("vtplanner_resource_crud", args=[slice_id, agg_id])),)
        ...
        print "[WARNING] Could not create resource in plugin 'vtplanner'. Details: %s" % str(e)
        DatedMessage.objects.post_message_to_user(
...
def manage_resource(request, resource_id, action_type):
    ...
    if action_type == "delete":
        VtPlannerController.delete(resource_id)
...
def get_sr_list(slice):
    return VtPlannerModel.objects.filter(slice_id = slice.uuid)

def get_sr_aggregates(slice):
    sr_aggs = []
    try:
        sr_aggs = slice.aggregates.filter(leaf_name=VtPlannerAggregateModel.__name__.lower())
    except:
        pass
    return sr_aggs
...
def get_ui_data(slice):
    ...
    try:
        ...
        print "[ERROR] Problem loading UI data for plugin 'vtplanner'. Details: %s" % str(e)
    return ui_context
