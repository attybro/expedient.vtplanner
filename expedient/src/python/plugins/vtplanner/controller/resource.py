"""
Controller for the VtPlanner object.
...
from vtplanner.utils.validators import validate_resource_name
import decimal, random

class VtPlanner():
    """
    Manages creation/edition of VtPlanner from the input of a given form.
    """
    ...
    def create(instance, aggregate_id, slice=None):
        try:
            ...
            if "Duplicate entry" in str(e):
                raise Exception("VtPlanner with name '%s' already exists." % str(instance.name))
            else:
                raise e
    ...
    def fill(instance, slice, aggregate_id, resource_id = None):
        ...
        if not resource_id:
            if VtPlannerModel.objects.filter(name = instance.name):
    ...
    def delete(resource_id):
        try:
            VtPlannerModel.objects.get(id = resource_id).delete()
        except:
            pass
