...
from vtplanner.forms.VtPlannerAggregate import VtPlannerAggregate as VtPlannerAggregateForm
#from sample_resource.forms.xmlrpcServerProxy import xmlrpcServerProxy as xmlrpcServerProxyForm
from vtplanner.forms.xmlrpcServerProxy import xmlrpcServerProxy as xmlrpcServerProxyForm
...
def aggregate_crud(request, agg_id=None):
    ...
    '''
    if agg_id != None:
        aggregate = get_object_or_404(VtPlannerAggregateModel, pk=agg_id)
    ...
    if request.method == "GET":
        agg_form = VtPlannerAggregateForm(instance=aggregate)
        client_form = xmlrpcServerProxyForm(instance=client)
        
    elif request.method == "POST":
        agg_form = VtPlannerAggregateForm(
        ...
        if client_form.is_valid() and agg_form.is_valid():
            ...
            addr='https://'+client.username+':'+client.password+'@'+client.url[8:]
            if (addr.endswith('/')):
				addr=addr[:-1]
            s = xmlrpclib.Server(addr)
            try:
                s.Ping('ping')
            except:
                errors = addr+"Could not connect to server: username, password or url are not correct"
...
def delete_resources(agg_id):
    resource_set = VtPlannerAggregateModel.objects.get(id = agg_id).resource_set.all()
...
def sync_am_resources(agg_id, xmlrpc_server):
    ...
    aggregate = VtPlannerAggregateModel.objects.get(id = agg_id)
...
#                SampleResourceController.create(instance, agg_id, slice)
                VtPlannerController.create(instance, agg_id)
        except:
            try:
        ...
        for node, node_connections in connections.iteritems():
            ...
            for connection in node_connections:
                try:
                    ...
                    res = VtPlannerModel.objects.get(name = connection)
                    if res:
            ...
            node_resource = VtPlannerModel.objects.get(name = node)
